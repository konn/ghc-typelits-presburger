...
decidePresburger _ref gs [] [] = do
  ...
  withTyCons $ do
    ...
    ngs <- mapM (\a -> runMachine $ (,) a <$> toPresburgerPred (subsType subst $ deconsPred a))
           gs
...
toPresburgerPred :: Given MyEnv => Type -> Machine Prop
...
toPresburgerPredTree :: Given MyEnv => PredTree -> Machine Prop
