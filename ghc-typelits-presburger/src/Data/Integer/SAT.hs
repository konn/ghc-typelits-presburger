...
getExprRange e (State s) =
  do
    let S m = expr e
        check (t, s1) = do
          l <- iTermBound Lower t (inerts s1)
...
expr (Min a b) = expr (If (a :<= b) a b)
expr (Max a b) = expr (If (a :<= b) b a)
expr (If p e1 e2) = do
  x <- newVar
  ...
  where
    (kickedOut, otherBounds) =
      ...
      let (mb, mp1) = Map.updateLookupWithKey (\_ _ -> Nothing) x (bounds i)
       ...
       in ( [ ct | (lbs, ubs) <- maybeToList mb, ct <- map (toCt Lower x) lbs ++ map (toCt Upper x) ubs
            ]
...
iTermBound bt (T k xs) is = do
  ks <- mapM summand (Map.toList xs)
  return $ sum $ k : ks
  where
  ...
  T (n1 + n2) $
    if Map.null m1
      then m2
        ...
        if Map.null m2
          then m1
          else Map.filter (/= 0) $ Map.unionWith (+) m1 m2
...
tGetSimpleCoeff (T a m) =
  do
    let (m1, m2) = Map.partition (\x -> x == 1 || x == -1) m
...
tLeastAbsCoeff (T c m) = do
  (xc, x, m1) <- Map.foldrWithKey step Nothing m
  return (xc, x, T c m1)
  where
